...
% dodać co nieco tutaj jeszcze. podać przykład lub odnieść się do opisanego
% wcześniej
...
%ciekawostka, pod tytułem a właściwie zwykli ludzie wykorzystują
% internet do swoich zwykłych, prozaicznych spraw, a przecież
% nie robią tego ze sztalugami czy płótnem malarskim


%\begin{itemize}
%%begin ewa
%\item hipertekst (nielinearna budowa, wielokierunkowa nawigacja)
%\item wirtulaność
%\item interaktywność (dwukierunkowa, nie tak jak na przykład w sztuce
% kinetycznej, gdzie dzieło odpowiadało na działania odbiorcy-uczestnika
% tutaj dzieło odpowiada, jest partnerem w interakcji, nie tylko odpowiada
% na działania, ale samo może działanie inicjować (przykład - rzeżbę kinetycz
% ną trzeba uruchomić, zainicjować interakcję i ją kontrolować w 100%
% a w sztuce internetu dzieło może również inicjować działania uczestnika
% prowokować go do reakcji, może nawet próbować przejmować kontrolę
% wszystko to oczywiście iluzja za pomocą mechanizmów stworzonych
% przez autora, jednak interakcja jest dwustronna
%\item hatywizm
%\item niematerialna (w def.)
%end ewa
%
%\item kluszczyński: otwartość, powszechność, bezgraniczność,
%artyści: akcentują wolność, anarchiczni są, zaangażowani politycznie
%i społecznie
%
%\item widz zmienia sie w współtworzącego dzieło (czasem nieświadomie
%tak jak w ippainting)
%\item podstawą jest komunikacja za pomocą Internetu (podstawowe medium)
%\item decentralizacja!
%\item wielokanałowa, równoległa, wieloprzebiegowa?, w czasie
%rzeczywistym, natychmiastowa komunikacja 
%\item temat to często Internet, medium, komunikacja
%\item czas! - zmienność, efemeryczność, dzieło-wydarzenie, jednostkowość
% i prywatność odbioru
%\item wykorzystanie narzędzi, technik, medium, które na co dzień nie są
%wykorzystywane w celu tworzenia sztuki, jest to nowość w stosunku
%do malarstwa czy rzeżby (było już w awangardzie oczywiście)
%
%\end{itemize}
%
%TODO: podzielić na cechy, które występują w każdym z dzieł net art
%a dalej dać cechy, które występują w niektórych (od najczęstszych do
%najbardziej lichych ;))
%
%Podsumowanie
